"""
NumMelee — server.py
Single round, no tiers. Flask backend for NumberGuess.vy on Celo.
...
.env:
  PRIVATE_KEY=0x...
  CONTRACT_ADDRESS=0x...
  RPC_URL=https://forno.celo.org        # or alfajores-forno
...
CONTRACT_ADDRESS = Web3.to_checksum_address(os.getenv("CONTRACT_ADDRESS", "0x" + "00"*20))
...
ABI = [
    {"name":"join","type":"function","inputs":[{"name":"commitment","type":"bytes32"}],"outputs":[],"stateMutability":"nonpayable"},
    {"name":"reveal_win","type":"function","inputs":[{"name":"winner","type":"address"},{"name":"number_scaled","type":"uint256"},{"name":"salt","type":"bytes32"}],"outputs":[],"stateMutability":"nonpayable"},
    {"name":"reveal_win_competitive","type":"function","inputs":[{"name":"winner","type":"address"},{"name":"medal","type":"uint8"},{"name":"number_scaled","type":"uint256"},{"name":"salt","type":"bytes32"}],"outputs":[],"stateMutability":"nonpayable"},
    {"name":"abort_round","type":"function","inputs":[],"outputs":[],"stateMutability":"nonpayable"},
    {"name":"get_round","type":"function","inputs":[],"outputs":[{"name":"round_id","type":"uint256"},{"name":"phase","type":"uint8"},{"name":"player_count","type":"uint256"},{"name":"opened_at","type":"uint256"},{"name":"started_at","type":"uint256"}],"stateMutability":"view"},
    {"name":"get_medals","type":"function","inputs":[{"name":"player","type":"address"}],"outputs":[{"name":"silver","type":"uint256"},{"name":"gold","type":"uint256"},{"name":"diamond","type":"uint256"}],"stateMutability":"view"},
    {"name":"is_joined","type":"function","inputs":[{"name":"player","type":"address"}],"outputs":[{"name":"","type":"bool"}],"stateMutability":"view"},
    {"name":"solo_time_remaining","type":"function","inputs":[],"outputs":[{"name":"","type":"uint256"}],"stateMutability":"view"},
    {"name":"RoundOpened","type":"event","inputs":[{"name":"round_id","type":"uint256","indexed":True},{"name":"opener","type":"address","indexed":False},{"name":"commitment","type":"bytes32","indexed":False}]},
    {"name":"PlayerJoined","type":"event","inputs":[{"name":"round_id","type":"uint256","indexed":True},{"name":"player","type":"address","indexed":True},{"name":"count","type":"uint256","indexed":False}]},
    {"name":"CompetitiveModeActivated","type":"event","inputs":[{"name":"round_id","type":"uint256","indexed":True},{"name":"count","type":"uint256","indexed":False}]},
    {"name":"RoundWon","type":"event","inputs":[{"name":"round_id","type":"uint256","indexed":True},{"name":"winner","type":"address","indexed":True},{"name":"medal","type":"uint8","indexed":False},{"name":"number_scaled","type":"uint256","indexed":False},{"name":"salt","type":"bytes32","indexed":False},{"name":"total_players","type":"uint256","indexed":False}]},
    {"name":"MedalAwarded","type":"event","inputs":[{"name":"player","type":"address","indexed":True},{"name":"medal","type":"uint8","indexed":True},{"name":"silver","type":"uint256","indexed":False},{"name":"gold","type":"uint256","indexed":False},{"name":"diamond","type":"uint256","indexed":False}]},
    {"name":"RoundAborted","type":"event","inputs":[{"name":"round_id","type":"uint256","indexed":True}]},
]
...
game = {
    "number_scaled":  None,
    "salt":           None,
    "commitment":     None,   # hex string without 0x
    "round_id":       0,
    "opened_at":      None,   # time.time() set by event poller
...
def gen_secret():
    ...
    commitment    = w3.keccak(encoded).hex()   # no 0x prefix
    return number_scaled, salt_hex, commitment

def send_tx(fn):
    if not op:
        raise RuntimeError("No PRIVATE_KEY set")
    nonce = w3.eth.get_transaction_count(op.address)
    tx    = fn.build_transaction({"from": op.address, "nonce": nonce,
...
def reset_game(round_id=0):
    game.update({"number_scaled": None, "salt": None, "commitment": None,
...
def _solo_timer(round_id, chain_opened_at):
    ...
    sleep_secs = max(0, deadline - time.time()) + 3
    time.sleep(sleep_secs)
    with lock:
        if game["round_id"] != round_id or game["is_competitive"]:
            return
    ...
    try:
        ...
        if phase < 2:
            ...
            print(f"[solo-timer] Round #{round_id} aborted → {receipt['transactionHash'].hex()[:14]}…")
            with lock:
                reset_game(round_id)
...
def start_solo_timer(round_id, chain_opened_at):
    threading.Thread(target=_solo_timer, args=[round_id, chain_opened_at], daemon=True).start()

# ── Win helpers ───────────────────────────────────────────────────────────────

def _reveal_solo(winner_addr):
    with lock:
        if not game["number_scaled"]: return
        ns, salt = game["number_scaled"], game["salt"]
    try:
        send_tx(contract.functions.reveal_win(Web3.to_checksum_address(winner_addr), ns, bytes.fromhex(salt)))
        print(f"[solo] reveal_win winner={winner_addr[:10]}…")
    except Exception as e:
        print(f"[solo] reveal_win FAILED: {e}")

def _reveal_competitive(winner_addr, medal):
    with lock:
        if not game["number_scaled"]: return
        ns, salt = game["number_scaled"], game["salt"]
    try:
        send_tx(contract.functions.reveal_win_competitive(
            Web3.to_checksum_address(winner_addr), medal, ns, bytes.fromhex(salt)))
        print(f"[competitive] reveal medal={MEDAL_NAMES[medal]} winner={winner_addr[:10]}…")
    except Exception as e:
    ...
    while True:
        try:
            ...
            if latest > _last_block:
                ...
                _evt_round_opened(fb, tb)
                _evt_player_joined(fb, tb)
                _evt_competitive(fb, tb)
                _evt_round_won(fb, tb)
                _last_block = latest
        except Exception as e:
            print(f"[poll] {e}")
        time.sleep(3)

def _evt_round_opened(fb, tb):
    try:
        ...
        print(f"[evt_round_opened] {e}")

def _evt_player_joined(fb, tb):
    try:
        for log in contract.events.PlayerJoined().get_logs(from_block=fb, to_block=tb):
            print(f"[event] PlayerJoined #{log['args']['round_id']} count={log['args']['count']}")
    except Exception as e:
        print(f"[evt_player_joined] {e}")

def _evt_competitive(fb, tb):
    try:
        for log in contract.events.CompetitiveModeActivated().get_logs(from_block=fb, to_block=tb):
            rid = log["args"]["round_id"]
            with lock:
        ...
        print(f"[evt_competitive] {e}")

def _evt_round_won(fb, tb):
    try:
        for log in contract.events.RoundWon().get_logs(from_block=fb, to_block=tb):
            rid = log["args"]["round_id"]
            print(f"[event] RoundWon #{rid} medal={MEDAL_NAMES[log['args']['medal']]} "
                  f"number={sc2f(log['args']['number_scaled'])}")
            with lock:
                if game["round_id"] == rid:
                    reset_game(rid)
    except Exception as e:
        print(f"[evt_round_won] {e}")
...
def api_commitment():
    with lock:
        ...
        if phase in (0, 1) and game["commitment"]:
            return jsonify({"commitment": "0x" + game["commitment"], "already_open": True})
...
def api_round():
    ...
    return jsonify({
        "round_id": round_id, "phase": phase, "player_count": player_count,
...
def api_guess():
    ...
    info = contract.functions.get_round().call()
    if info[1] not in (0, 1):
        return jsonify({"error": "round not active"}), 400

    if info[1] == 0 and info[3] > 0:
...
def api_cooldown():
    ...
    return jsonify({"remaining": round(max(0, COOLDOWN - (time.time() - cooldowns.get(player, 0))), 2)})

@app.route("/api/leaderboard")
def api_leaderboard():
    try:
        from_block = max(0, w3.eth.block_number - 50_000)
        logs = contract.events.MedalAwarded().get_logs(from_block=from_block, to_block="latest")
        pm = {}
        for log in logs:
            addr = log["args"]["player"].lower()
            pm[addr] = {"address": log["args"]["player"], "silver": log["args"]["silver"],
                        "gold": log["args"]["gold"], "diamond": log["args"]["diamond"]}
        ranked = sorted(pm.values(), key=lambda x: (x["diamond"], x["gold"], x["silver"]), reverse=True)[:50]
...
def admin_status():
    ...
    info = contract.functions.get_round().call()
    with lock:
        return jsonify({"round_id": info[0], "phase": info[1], "player_count": info[2],
...
if __name__ == "__main__":
    print(f"NumMelee backend  |  contract={CONTRACT_ADDRESS}  |  rpc={RPC_URL}")
    if op: print(f"Operator: {op.address}")
    else:  print("WARNING: No PRIVATE_KEY — owner txs will fail")
